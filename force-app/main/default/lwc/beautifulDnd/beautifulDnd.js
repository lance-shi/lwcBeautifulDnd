...
export default class BeautifulDnd extends LightningElement {
    ...
    handleItemDrop(evt) {
        ...
        let lists = this.template.querySelectorAll("c-dnd-list");
        for(let curList of lists) {
            curList.resetPaddings();
        }
    }
    ...
    handleMouseHover(evt) {
        let lists = this.template.querySelectorAll("c-dnd-list");
        for(let curList of lists) {
            if(curList.category !== evt.detail.category) {
                curList.handleCollide(evt.detail.posX, evt.detail.posY, evt.detail.category);
            }
        }
    }
}